...
import { 
  ...
  Loader2, ArrowRight, Check, AlertTriangle 
} from 'lucide-react';
...
const Navbar: React.FC<NavbarProps> = ({ activePage, onNavigate }) => {
  ...
  const showToast = (message: string, type: 'success' | 'error' = 'success', subMessage = '') => {
    ...
    }, 4500); // Sedikit lebih lama biar user sempat baca
  };
  ...
  const handleCheckout = async () => {
    ...
    try {
      const orderId = `GDR-${Math.floor(10000 + Math.random() * 90000)}`; // Ganti prefix jadi GDR biar keren
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between h-10">
            ...
            <div className="flex-shrink-0 cursor-pointer z-50 relative" onClick={() => handleNavClick('home')}>
               <img 
                src="/images/logo.png" 
                alt="Gdrone Logo" 
                className={`transition-all duration-300 w-auto object-contain h-12 md:h-16 ${showSolidNav ? '' : 'brightness-0 invert'}`} 
              />
            </div>
            ...
            <div className="hidden lg:flex items-center space-x-2">
              ...
              <button onClick={() => setIsOpen(true)} className={`ml-2 px-4 py-2 text-xs font-bold uppercase tracking-wider rounded bg-slate-200/20 backdrop-blur hover:bg-slate-200/40 transition-all flex items-center gap-2 ${textColorClass}`}>
                Menu
              </button>
            </div>
      ...
      <div className={`fixed inset-0 bg-white z-[60] transition-all duration-500 ease-in-out flex flex-col ${isOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-5'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 w-full">
              ...
              <div className="hidden md:flex flex-col justify-start items-start space-y-8 pl-12 border-l border-gray-100 pt-4">
                 ...
                 <div className="grid grid-cols-2 gap-8 w-full">
                    <div><h4 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Kontak</h4><p className="text-sm font-medium text-slate-900">+62 21 5555 8888</p><p className="text-sm text-slate-500">hello@gdrone.id</p></div>
                    <div><h4 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Lokasi</h4><p className="text-sm font-medium text-slate-900">Jakarta Selatan, ID</p></div>
                 </div>
              </div>
            </div>
          </div>
        ...
        <div className="border-t border-gray-100 bg-gray-50 flex-shrink-0">
           <div className="max-w-[1400px] mx-auto px-6 py-6 flex justify-between items-center">
             ...
             <div className="flex space-x-6 text-xs font-bold uppercase tracking-widest text-slate-500"><button>Instagram</button><button>Twitter</button></div>
           </div>
        </div>
      </div>
      ...
      {/* --- PREMIUM HUD NOTIFICATION (DESAIN BARU) --- */}
      <div 
        className={`fixed top-28 right-4 md:right-8 z-[100] transition-all duration-500 ease-out transform ${
          notification.show ? 'translate-x-0 opacity-100' : 'translate-x-10 opacity-0 pointer-events-none'
        }`}
      >
        <div className={`
          relative overflow-hidden w-80 md:w-96 rounded shadow-2xl border
          ${notification.type === 'success' ? 'bg-slate-900 border-slate-700' : 'bg-slate-900 border-red-900'}
        `}>
          ...
          <div className="p-5 flex items-start gap-4">
            ...
            <div className="flex-1">
              <div className="flex justify-between items-start">
                ...
                <button onClick={() => setNotification(prev => ({...prev, show: false}))} className="text-slate-500 hover:text-white transition-colors">
                  <X size={14} />
                </button>
              </div>
